...
在React里面，传参要看传的只是event{\color{red}还是含有} (event貌似是一定有的) 其它参数 (包括其它参数为空的情形)。当为其它参数的时候，有以下几种写法 (可以参考ylx-pc门票列表)，箭头函数当用于事件触发的地方时能替代bind，当class methods使用的时候就{\color{red}不}可以替代bind
\begin{enumerate}
\item \begin{verbatim}
handleOnClick(item) {
