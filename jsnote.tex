...
\subsection{git tag}
\verb|git tag -a Developing -m "getting started" 18f7c64|
...
\item How to view a file at a specific commit in git?\\
\verb|git show REVISION:/path/to/file|

You can also save a copy by\\
\verb|git show REVISION:/path/to/file >file.copy|
...
\item \verb|[Vue warn]: You are using the runtime-only build of Vue where the|\\ \verb|template compiler is not available|

\url{https://blog.csdn.net/weixin_42009735/article/details/85260932}

\item 端口占用的错误

首先cmd里 \verb|netstat -ano| 找到对应端口的 PID，然后运行 \verb|TASKKILL /F /PID 76532| 最后的一串数字替换为那个 PID

\item \verb|E:\Javascript\learningVuejs\imooc\todo-list|项目运行不成功没有用，但是也没有删除，在另外一个webpack-demo-app项目的基础上实践的。等过了10天就忘了当初运行不成功，然后就陷入混乱状态。今天（8.27日）把那个文件夹删了
...
\subsection{\href{https://juejin.im/post/6844904178901319687}{vue插槽slot}}


\section{\href{https://cn.vuejs.org/v2/guide/}{vuejs 官方教程}}

\subsection{\href{https://cn.vuejs.org/v2/api/index.html}{API}}
\verb|全局配置|\\
...
\verb|选项 / 数据|\\ 
\indent\verb|选项 / DOM|\\ 
\indent\verb|选项 / 生命周期钩子|\\ 
\indent\verb|选项 / 资源|\\ 
\indent\verb|选项 / 组合|\\ 
\indent\verb|选项 / 其它|\\ 

\indent\verb|实例 property|\\
\indent\verb|实例方法 / 数据|\\ 
\indent\verb|实例方法 / 事件|\\ 
\indent\verb|实例方法 / 生命周期|\\

\verb|指令| 指令的参数就是冒号后面的那个单词\\
...
在\ 选项/DOM 中使用\ 选项/数据\ 的时候不用加 this，但是\ 选项/数据\ 里的互相调用时需要加 this？

\subsection{Vue 实例}
当一个 Vue 实例被创建时，它将 \verb|data| 对象中的所有的 property 加入到 Vue 的\textbf{响应式系统}中。
...
\verb|<a v-bind:href="url">...</a>|

在这里 \verb|href| 是参数，告知 \verb|v-bind| 指令将该元素的 \verb|href| attribute 与表达式 \verb|url| 的值绑定。
...
\subsection{计算属性和侦听器 (computed 和 watch)}
对于任何复杂逻辑，你都应当使用计算属性。即 api 里 选项 / 数据 的 \verb|computed|

\subsubsection{侦听器}
当需要在数据变化时执行异步或开销较大的操作时，这个方式是最有用的。

\subsection{Class 与 Style 绑定}

\subsection{列表渲染}
\subsubsection{在 v-for 里使用对象}
...
\subsection{表单输入绑定}
你可以用 v-model 指令在表单 \verb|<input>|、\verb|<textarea>| 及 \verb|<select>| 元素上创建双向数据绑定。
...
\textbf{在组件上使用 v-model} part seems important，可以参考 \href{https://cn.vuejs.org/v2/guide/components-custom-events.html}{自定义组件的 v-model}


\subsection{深入了解组件}

\subsubsection{组件注册}
当直接在 DOM 中（的模板）使用一个组件 (而不是在字符串模板 template 或单文件组件) 的时候，我们强烈推荐遵循 W3C 规范中的自定义组件名 (字母全小写且必须包含一个连字符)，可以参考\href{https://cn.vuejs.org/v2/style-guide}{基础组件名}的“详解”的“这样做的几个好处”部分

\subsubsection{Prop}
\begin{itemize}
\item \textbf{Prop 的大小写 (camelCase vs kebab-case)}

使用 DOM 中的模板时，camelCase (驼峰命名法) 的 prop 名需要使用其等价的 kebab-case (短横线分隔命名) 命名

\item \textbf{传递静态或动态 Prop}

像这样给 prop 传入一个静态的值：\\
\verb|<blog-post title="My journey with Vue"></blog-post>|\\
...
\item \textbf{非 Prop 的 Attribute} 里的"禁用 Attribute 继承"部分不理解

\end{itemize}

\subsubsection{自定义事件}
\textbf{将原生事件绑定到组件} 没有完全搞透

\textbf{.sync 修饰符} 没有完全搞透：

注意里的，\verb|v-model| 不能和表达式一起使用吗？

\subsubsection{插槽}
\textbf{独占默认插槽的缩写语法}: 当被提供的内容只有默认插槽时，组件的标签可以被当作插槽的模板来使用
...
\item if returned array retain the element (based on the return value), then whether it is the same as the original array or a modified version of the original array (sort() returns the modified version of the original array, which means two things, it returns an array and it also modify the original array)
\end{enumerate}
...
\href{https://javascript.info/nullish-coalescing-operator#summary}{Summary} section is very good.
...
Sometimes \verb|isFinite| is used to validate whether a string value is a regular number
...
\href{https://javascript.info/number#parseint-and-parsefloat}{parseInt and parseFloat} part is short and clear
...
\subsubsection{Strings (finished including tasks)}
The internal format for strings is always UTF-16, it is not tied to the page encoding.

\subsubsection{Arrays}
\textbf{A word about “length”: }the simplest way to clear the array is: \verb|arr.length = 0;|

\href{https://javascript.info/array#calling-in-an-array-context}{Calling in an array context} solution explanation is good

\href{https://javascript.info/array#sum-input-numbers}{Sum input numbers}
\begin{itemize}
\item Asks the user for values using \verb|prompt| and stores the values in the array
\item Finishes asking when the user enters a non-numeric value, an empty string, or presses “Cancel”
\end{itemize}

Unfinished task:

\href{https://javascript.info/array#a-maximal-subarray}{A maximal subarray}

\subsubsection{Array methods}
\textbf{slice}: We can also call it without arguments: \verb|arr.slice()| creates a copy of \verb|arr|. That's often used to obtain a copy for further transformations that should not affect the original array.

\textbf{find and findIndex.} In real life arrays of objects is a common thing, so the \verb|find| method is very useful.

\textbf{sort(fn)}: The items are sorted as strings by default.

\textbf{reduce/reduceRight}: If the array is empty, then \verb|reduce| call without initial value gives an error. So it's advised to always specify the initial value.

\href{https://javascript.info/array-methods#summary}{Summary} section is good. Please note that methods \verb|sort|, \verb|reverse| and \verb|splice| modify the array itself.

Task \href{https://javascript.info/array-methods#create-an-extendable-calculator}{Create an extendable calculator} is worth noting.

unfinished parts and after:\\ \url{https://javascript.info/array-methods#shuffle-an-array}

\subsubsection{Iterables}
\textit{Iterable} objects is a generalization of arrays. That's a concept that allows us to make any object useable in a \verb|for..of| loop.

\textbf{Iterables and array-likes}
\begin{itemize}
\item \textit{Iterables} are objects that implement the \verb|Symbol.iterator| method, as described above.

\item \textit{Array-likes} are objects that have indexes and \verb|length|, so they look like arrays.
...
\noindent\href{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules}{JavaScript modules} (finished but not \textbf{See also} part): 
\begin{itemize}
\item You can only use import and export statements inside modules, not regular scripts.

\item we are using the dot (.) syntax to mean "the current location"
\end{itemize}
...
\section{Web API (browser)}
\verb|document.getElementById()| returns an HTMLElement Object which inherits from Element object.

\subsection{chrome}
\begin{enumerate}
\item In the developer console, press \verb|shift+enter| to enter multiple lines code

\item \href{https://developers.google.com/web/updates/2018/04/devtools#stop}{Stop infinite loops}
\end{enumerate}
...
\item CSS Animations. 接下来学习 Specify the Speed Curve of the Animation
...
Since the result of using the \verb|box-sizing: border-box;| is so much better, many developers want all elements on their pages to work this way.

\item CSS Flexbox.

The flex container properties are:
\begin{itemize}
\item \verb|flex-direction|
\item \verb|flex-wrap|
\item \verb|flex-flow|
\item \verb|justify-content|
\item \verb|align-items|
\item \verb|align-content|
\end{itemize}
The \verb|flex-flow| property is a shorthand property for setting both the \verb|flex-direction| and \verb|flex-wrap| properties.

The flex item properties are:
\begin{itemize}
\item \verb|order|
\item \verb|flex-grow|
\item \verb|flex-shrink|
\item \verb|flex-basis|
\item \verb|flex|
\item \verb|align-self|
\end{itemize}
The \verb|flex| property is a shorthand property for the \verb|flex-grow|, \verb|flex-shrink|, and \verb|flex-basis| properties. \textbf{Note:} It is \textit{strongly} recommended that authors use this property instead of the separate properties it encompasses.

The \verb|align-self| property overrides the default alignment set by the container's \verb|align-items| property.
...
\subsection{配置小程序 (finished)}
\href{https://developers.weixin.qq.com/miniprogram/dev/framework/sitemap.html}{sitemap 配置} 里的 “页面被索引” 是什么意思？
...
单独说 API 的时候指\href{https://developers.weixin.qq.com/miniprogram/dev/framework/app-service/api.html#API}{微信原生 API}，详细介绍请参考 \href{https://developers.weixin.qq.com/miniprogram/dev/api/}{API 文档}

\href{https://developers.weixin.qq.com/miniprogram/dev/reference/api/App.html}{App 参考文档}

\textbf{模块化}: exports 是 module.exports 的一个引用

\textbf{WXML}: 接下来学习 \url{https://developers.weixin.qq.com/miniprogram/dev/reference/wxml/list.html}
...
\href{https://webpack.js.org/configuration/module/#ruletest}{Rule.test} 可参考 \href{https://webpack.js.org/configuration/module/#condition}{Condition}，注意 string 和 RegExp 的区别

\subsection{\href{https://www.youtube.com/watch?v=MpGLUVbqoYQ}{Learn Webpack - Full Tutorial for Beginners}}
为什么在 production 的模式下 filename 添加 contentHash? (55:40)

这个如果上线了（网站放在了服务器上），如果做了更改重新上传，因为名字不加 contentHash 就一直是 main.js，那么用户的浏览器可能认为已经 cache 了相同名字的文件则刷新的时候可能就不是最新的了（可以做一个上线的项目实战一下）

\section{\href{https://survivejs.com/webpack/}{SURVIVEJS — WEBPACK}}

\subsection{\href{https://survivejs.com/webpack/what-is-webpack/}{What is Webpack}}
\begin{itemize}
...
webpack treats all code as JavaScript by default
\end{itemize}

\subsection{\href{https://survivejs.com/webpack/developing/}{Developing}}
\subsubsection{Getting Started}
\begin{itemize}
\item\textbf{Executing Webpack}

试了一下 bottom part, not working，我在 package.json 文件里 "scripts" 对象里声明　\verb|"start": "webpack --mode development"| 基本上可以重现这节的内容

\item \textbf{Adding a Build Shortcut}

\textbf{Note:} There are shortcuts like npm start and npm test. You can run these directly without npm run although that works too. For those in a hurry, you can use npm t to run your tests.
...
\item \textbf{Webpack watch Mode and webpack-dev-server}

WDS is a development server running \textbf{in-memory}, meaning the bundle contents aren't written out to files but stored in memory.

\item \textbf{Enabling Error Overlay}

试了一下，not working. 因为 WDS overlay does not capture runtime errors of the application. 如果加一个 compile time error 比如 \verb|console.log("foo)|，那么就有了。

\end{itemize}

\subsection{\href{https://survivejs.com/webpack/styling/}{Styling}}
\subsubsection{Loading Styles}
\begin{itemize}
\item To use source maps, you have to enable \verb|sourceMap| for each style loader you are using except for \textbf{style-loader} and you should set \verb|output.publicPath| to an absolute url that points to your development server. 这句话里 and 后面说的没有实践

%\item 

\end{itemize}

\subsubsection{Separating CSS}
到 \textbf{Connecting with configuration} 实践的时候报错了\\ \verb|Error: No loader specified|

安装了 less-loader 作为参数传到 \verb|parts.extractCSS()| 将 \verb|test: /\.css$/| 改为 \verb|test: /\.less$/|，修改 main.css 为 main.less 和 index.js 的引入路径后文件测试成功。

\textbf{Question:} how to debug webpack.config.js?

\subsection{\href{https://survivejs.com/webpack/loading/}{Loading Assets}}
\subsubsection{Loading Images} \href{https://survivejs.com/webpack/loading/images/#loading-svgs}{Loading SVGs},
我想 devServer.writeToDisk 看效果，但是没成功

\subsubsection{Loading JavaScript}
Presets are collections of plugins, and you can define your own as well.

babel-loader takes the code and turns it into a format older browsers can understand.

\verb|@babel/preset-env| is valuable as it can choose which features to compile and which polyfills to enable based on your browser definition.

Webpack 5 includes TypeScript support out of the box

\subsection{\href{https://survivejs.com/webpack/building/}{Building}}
In addition to source compiling to JavaScript, this works for styling as well.

\subsection{\href{https://survivejs.com/webpack/optimizing/}{Optimizing}}
\subsubsection{Environment Variables}
Starting from webpack 4, \verb|process.env.NODE_ENV| is set within the build based on the given mode but not globally. To pass the variable to other tools, you'll have to set it explicitly outside of webpack or within webpack configuration.

\subsubsection{Performance}
Often it's a good idea to start with more accessible techniques before moving to more involved ones.
...
normal flow & 普通流 & statement & 语句 & expression & 表达式 \\
% &  &  &  &  & \\
\end{tabular}
