...
\item 如果当前页面有从localStorage里面取数据，那么就不能直接在url上面写参数 (比如goodsTicketId)，因为请求的数据会跟从localStorage里面取的数据不一样导致混淆
...
The \verb|await| keyword can only be used inside an \verb|async| function. \verb|await| is an operator: it returns the resolved value of a promise. Since promises resolve in an indeterminate amount of time, \verb|await| halts, or pauses, the execution of our \verb|async| function until a given promise is resolved.
